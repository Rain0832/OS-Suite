...
static void notify_worker()
{
    ...
    ssize_t bytes_written = write(pipe_write, tests, sizeof(tests));
    if (bytes_written == -1)
    {
        // 处理写入失败的情况
        perror("write failed");
    ...
    else if (bytes_written != sizeof(tests))
    {
        // 处理部分写入的情况
        fprintf(stderr, "write: only wrote %zd out of %zu bytes\n", bytes_written, sizeof(tests));
        // 可能需要重新尝试写入剩余的部分
    }
    close(pipe_write);
